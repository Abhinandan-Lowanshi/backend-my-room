...
exports.Tofavorite = async (req, res) => {
    try {
        ...
        if (!user_id || user_id == '' || user_id === undefined || !room_id || room_id == '' || room_id === undefined || fav_type === undefined) {
            return res.json({
                status: false,
        ...
        if (fav_type == 1) {
            ...
            if (existdata) {
                return res.json({ status: false, message: 'Room already added to favorite list.' })

            }

            await favorite.create({ fav_usr_fkey: user_id, fav_rm_fkey: room_id })
                .then((result) => {
                    ...
                    return res.json({ status: true, message: 'Room added to favorite list successfully.' })
            ...
            await favorite.destroy({
                ...
                if (result) {
                    return res.json({ status: true, message: 'Room removed to favorite list successfully.' })
                }

                return res.json({ status: true, message: 'Room details not found in favorite list.' })
