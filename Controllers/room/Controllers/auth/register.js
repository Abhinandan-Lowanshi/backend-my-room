...
exports.sendVerifyMail = async (req, res) => {
    try {
        ...
        if (!emailExists) {
            ...
            await temp.create({ emailOTP: otp, email: email })
                .then(result => {
...
exports.EmailVerifyOtp = async (req, res) => {
    try {
        ...
        const result = await temp.findOne({ where: { email: email.toLowerCase(), emailOTP: otp } });
...
exports.register = async (req, res) => {
    try {
        const { firstName, lastName, email, phone, currentAdrs, prmntAddress, password, device_token } = req.body;
        ...
        const reqData = {
            usr_firstName: firstName, usr_lastName: lastName, usr_email: email.toLowerCase(),
            usr_phone: phone, usr_parmentAdrss: prmntAddress, usr_currentAdrss: currentAdrs, usr_pasword: encoded,
            device_token
        }
        ...
        if (emailExists) {
            res.json({ status: false, message: "Email already registered.", data: {} });
        } else {
            ...
            User.create(reqData)
                .then(async (ressult) => {
                    ...
                    const token = await jwtHelper.signAccessToken({ email: email, userId: ressult.usr_pkey })
                    res.json({
                        status: true,
                        message: 'Register successfully.',
                        token,
                        data: {
                            "usr_id": ressult.usr_pkey,
                            "usr_firstName": ressult.usr_firstName,
                            "usr_lastName": ressult.usr_lastName,
                            "usr_email": ressult.usr_email,
                            "usr_phone": ressult.usr_phone,
                            "usr_parmentAdrss": ressult.usr_parmentAdrss,
                            "usr_currentAdrss": ressult.usr_currentAdrss
                        },
                    })
                })
                .catch((err) => { console.log(err); res.json({ status: true, message: 'Something went wronge.', data: {}, orignalError: err }) })
        }
    }
    catch (err) { console.log(err); res.json({ status: false, message: 'Something went wronge.', data: {}, orignalError: err }) }
}
